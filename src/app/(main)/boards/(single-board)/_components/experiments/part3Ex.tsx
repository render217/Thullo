...
export default function Part3Ex({ board }: { board: IBoard }) {
...
function SortableTaskItem({
  ...
  function handleCardDragEnd2(event: DragEndEvent) {
    const { active, over } = event;
    if (active?.id !== over?.id) {
      setTasks((prev) => {
        const activeTaskIndex = prev.findIndex((task) => task.id === item.id);
        const activeIndex = prev[activeTaskIndex].cards.findIndex(
          (card) => card.id === active?.id,
        );
        const overIndex = prev[activeTaskIndex].cards.findIndex(
...
function SortableCardOverlay({ card }: { card: ICard }) {
  ...
  return (
    <div
      ...
      {...listeners}
      className={cn(
        "h-[200px] rotate-3 rounded-md border-2 border-gray-200 bg-white px-2 text-sm hover:cursor-pointer hover:border-gray-600",
      )}
    >
      {card.title}
    </div>
